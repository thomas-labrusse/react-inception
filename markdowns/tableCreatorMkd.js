...
export const mkdInitializer = `const initializeState = (format) => {
   if (format === 2) {
      return [
         ['', ''],
         ['', ''],
...
export const mkdUseReducer = `const [state, dispatch] = useReducer(reducer, format, initializeState)`

export const mkdUseReducerNoInitializer = `const [state, dispatch] = useReducer(reducer,initialState)`

export const mkdHandler = `const handleEditCell = (colId, rowId, content) => {
   dispatch({
      type: 'edit_cell',
      colId: colId,
      rowId: rowId,
...
export const mkdReducer1 = `case 'edit_cell': {
   const newArray = [...state]
   newArray[action.colId][action.rowId] = action.content
   return [...newArray]
}`
